...
> **No `storageClassName` set:** omitting the field lets the DataVolumeTemplate use your cluster's default StorageClass automatically. Do **not** set `storageClassName: ""` — that explicitly disables dynamic provisioning and requires a pre-existing PV, which causes `no persistent volumes available for this claim and no storage class is set`. Only add `storageClassName: <name>` here if you need a specific non-default class (check available classes with `oc get storageclass`).
