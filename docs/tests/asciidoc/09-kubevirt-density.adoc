...
NOTE: kube-burner v2.7.3 automatically creates a temporary DaemonSet to pre-pull the container image to all nodes before starting the test. This appears as `Pre-load: All images pulled on N nodes` and adds ~15 seconds but ensures consistent boot times. It is removed automatically.
